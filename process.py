...
def vision_preproc(x) -> torch.Tensor | None:
    ...
    print(f"[DEBUG] vision_preproc input: type={type(x)}, shape={x.shape if hasattr(x, 'shape') else 'no shape'}")
    ...
    if isinstance(x, np.ndarray):
        print(f"[DEBUG] vision_preproc: converting numpy array, shape={x.shape}, dtype={x.dtype}")
    ...
    print(f"[DEBUG] vision_preproc: tensor stats - min={x.min()}, max={x.max()}, mean={x.mean()}")
...
class SequentialProcessor:
    ...
    def _init_zmq(self):
        ...
        self.publisher.connect(f"tcp://{self.cfg['robot']['pub']['ip']}:{self.cfg['robot']['pub']['port']}")
    ...
    def _init_governor(self):
        ...
        try:
            # Movement symbols
            self.movement_symbols = ["moveup", "movedown", "moveleft", "moveright", "idle", "standup"]
            ...
            self.governor_optimizer = torch.optim.Adam(self.titans_model.parameters(), lr=0.001)
            self.ctx = torch.zeros((1, self.ctx_len, self.emb_dim), dtype=torch.float32).to(self.device)
    ...
    def perceive_vision(self, vision_data) -> torch.Tensor | None:
        ...
        print(f"[DEBUG] perceive_vision called with vision_data type: {type(vision_data)}")
        ...
        print(f"[DEBUG] vision_preproc returned: {type(x)}, shape: {x.shape if x is not None else 'None'}")
        ...
        try:
            ...
            if latent is None:
                ...
                if distances:
                    ...
                    padded_distances = (distances + [100.0] * self.emb_dim)[:self.emb_dim]
                    latent = torch.tensor(padded_distances, dtype=torch.float32).to(self.device) / 100.0
                    latent = latent.unsqueeze(0)  # Add batch dimension
                else:
                    return {"emotion_guidance": None}
            
            # Create dummy emotion (neutral state)
            dummy_emotion = torch.zeros(3, dtype=torch.float32).to(self.device)  # PAD format [P, A, D]
            
            # Create dummy governance (equal weights for all intrinsics)
            dummy_governance = torch.ones(len(self.intrinsics), dtype=torch.float32) / len(self.intrinsics)
            ...
            information = {
                ...
                "force_on_geoms": torch.zeros(1, dtype=torch.float32).to(self.device)  # Backward compatibility
            }
            ...
            for intrinsic_name in self.intrinsics:
                if intrinsic_name in self.motivators:
                    ...
                    try:
                        ...
                        intrinsic.impl(information, {}, None)  # No brain_shm, no physics
            ...
            return {
                ...
                "agent_state": agent_state,
                "threats_detected": detection_summary.get("trap", {}).get("count", 0) + detection_summary.get("enemy", {}).get("count", 0),
                "goals_detected": detection_summary.get("goal", {}).get("count", 0) + detection_summary.get("food", {}).get("count", 0)
            }
    ...
    def _categorize_detections(self, raycast_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        categorized = {name: {"count": 0, "distances": [], "angles": []} for name in type_mapping.values()}
        ...
        try:
            ...
            if latent is None:
                ...
                if distances:
                    ...
                    latent = torch.zeros(1, self.emb_dim, dtype=torch.float32).to(self.device)
            else:
        ...
        try:
            ...
            if self.subscriber.poll(timeout=100):  # 100ms timeout
                ...
                output = {
                    ...
                    "raycast_count": len(context.get("raycast_info", {}).get("distances", [])),
                    "emotion_guidance_available": motivator_info.get("emotion_guidance") is not None,
                    "detection_summary": context.get("detection_summary", {}),
                    "agent_status": {
    ...
    def _construct_vision_from_raycast(self, line_of_sight: List[Dict[str, Any]]) -> np.ndarray:
            ...
            for row in range(kernel_size//2, height - kernel_size//2):
                for col in range(kernel_size//2, width - kernel_size//2):
                    ...
                    if np.any(neighborhood > 0):
                        smoothed_channel[row, col] = np.mean(neighborhood[neighborhood > 0]) * 0.7
    ...
    def _construct_polar_vision_from_raycast(self, line_of_sight: List[Dict[str, Any]]) -> np.ndarray:
